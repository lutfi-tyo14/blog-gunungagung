  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-gradient-to-b from-green-200 via-blue-100 to-white relative overflow-hidden">
      ...
      <form onSubmit={handleRegister} className="flex flex-col gap-4 w-full max-w-xs bg-white/80 rounded-xl shadow-lg p-6 mt-4">
        ...
        <button type="submit" className="bg-gradient-to-r from-green-500 to-blue-600 text-white py-2 rounded-xl shadow-lg hover:scale-105 hover:from-green-600 hover:to-blue-700 transition-all text-lg font-bold tracking-wide">Daftar</button>
      </form>
      {success && (
      ...
      <button className="mt-4 text-blue-600 underline" onClick={() => router.push("/auth/login")}>Sudah punya akun? <span className="font-semibold">Masuk</span></button>
    </div>
  );
} 